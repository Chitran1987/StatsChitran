##Write a function to compute Riemann sums between two fixed values
num.integrate.riemann <- function(X, Y, xmin, xmax){
  ##error handling
  if(xmin < min(X)){
  ...
  if(length(X) <= 5*10^5){
    s = RSum(X, Y, xmin, xmax)
  }else{
    s = RSum_par(X, Y, xmin, xmax)
  }
  return(s)
}
